...
由于 Rust 没有继承，因此无法像`rclcpp`或`rclpy`中常见做法那样从`Node`继承。


ROS2需要的依赖目前都不存在于[crates.io](https://crates.io/)官方网站上。而 colcon 通过编译 ROS2_Rust 依赖项和 ROS2_Rust 项目，将 cargo 的依赖项搜索直接重定向到本地的`workspace/install`目录，在那里它会找到本地生成的 Rust 项目作为依赖项使用。几乎所有的消息类型都以这种本地依赖的形式存在。
